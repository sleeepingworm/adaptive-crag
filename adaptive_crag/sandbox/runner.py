...
class SandboxRunner:
    ...
    def execute(
        ...
        """核心执行方法"""
        print(f"[LOG] [Sandbox] execute() 入口 — 代码长度={len(code)} 字符, 数据文件数={len(data_files or [])}")
        # 安全检查
        is_safe, reason = check_code_safety(code, self.restricted_modules, self.allowed_packages)
        if not is_safe:
            print(f"[LOG] [Sandbox] 安全校验失败: {reason}")
        ...
        try:
            ...
            print(f"[LOG] [Sandbox] 开始执行子进程 — timeout={timeout}s")
            try:
                ...
                print(f"[LOG] [Sandbox] 执行完毕 — success={artifact.success}, 耗时={elapsed_ms}ms")

            except subprocess.TimeoutExpired:
                elapsed_ms = int((time.time() - start_time) * 1000)
                print(f"[LOG] [Sandbox] 执行超时 — {timeout}s")
                artifact = ExecutionArtifact(
                    success=False,
