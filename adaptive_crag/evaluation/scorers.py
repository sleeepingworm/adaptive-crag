...
class Scorer:
    ...
    def web_search_trigger_accuracy(results: list[dict]) -> float:
        ...
        需要 result 中包含 web_search_triggered 和 web_search_needed 字段。
        当前 BenchmarkRunner 未记录这些字段，返回 0.0。
        """
        triggered = [r for r in results if r.get("web_search_triggered")]
        if not triggered:
            return 0.0
        correctly_triggered = sum(
            1 for r in triggered if r.get("web_search_needed", False)
        )
        return correctly_triggered / len(triggered)
